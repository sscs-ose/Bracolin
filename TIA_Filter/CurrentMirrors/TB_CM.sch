v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
B 2 110 0 910 400 {flags=graph
y1=-2.3e-12
y2=2.5e-23
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=3.3
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0
node="i(v1)
i(v2)"
color="4 5"
dataset=-1
unitx=1
logx=0
logy=0
rainbow=1}
B 2 1260 -20 2060 380 {flags=graph
y1=1e-12
y2=2.4e-12
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=3.3
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0
rainbow=1
color="4 5"
node="i(v13)
i(v14)"}
N -400 -220 -400 -200 {
lab=GND}
N -400 -310 -400 -280 {
lab=VDD}
N -420 -310 -400 -310 {
lab=VDD}
N 110 -260 110 -200 {
lab=INn}
N 150 -210 150 -190 {
lab=o1}
N 170 -210 170 -190 {
lab=o2}
N 210 -210 210 -190 {
lab=o3}
N 230 -210 230 -190 {
lab=o4}
N 270 -210 270 -190 {
lab=o5}
N 330 -210 330 -190 {
lab=o7}
N 350 -210 350 -190 {
lab=o8}
N 390 -210 390 -190 {
lab=o9}
N 410 -250 410 -230 {
lab=o10}
N 450 -210 450 -190 {
lab=o11}
N 470 -250 470 -230 {
lab=o12}
N 180 -390 180 -350 {
lab=Voutn}
N -310 -220 -310 -200 {
lab=GND}
N -330 -310 -310 -310 {
lab=Voutn}
N -310 -310 -310 -280 {
lab=Voutn}
N 410 -230 410 -190 {
lab=o10}
N 470 -230 470 -190 {
lab=o12}
N 180 -350 180 -340 {
lab=Voutn}
N 110 -200 110 -190 {
lab=INn}
N 60 -100 80 -100 {
lab=GND}
N 170 -250 170 -210 {
lab=o2}
N 230 -250 230 -210 {
lab=o4}
N 350 -250 350 -210 {
lab=o8}
N 150 -310 150 -270 {
lab=Voutn}
N 150 -310 170 -310 {
lab=Voutn}
N 180 -340 180 -310 {
lab=Voutn}
N 150 -270 390 -270 {
lab=Voutn}
N 170 -310 410 -310 {
lab=Voutn}
N 290 -250 290 -240 {
lab=o6}
N 410 -310 470 -310 {
lab=Voutn}
N 390 -270 450 -270 {
lab=Voutn}
N 1170 -530 1220 -530 {
lab=VDD}
N 1290 -440 1290 -160 {
lab=#net1}
N 1310 -440 1310 -340 {
lab=#net2}
N 1350 -440 1350 -160 {
lab=#net3}
N 1370 -440 1370 -340 {
lab=#net4}
N 1410 -440 1410 -160 {
lab=#net5}
N 1430 -440 1430 -340 {
lab=#net6}
N 1470 -440 1470 -160 {
lab=#net7}
N 1490 -440 1490 -340 {
lab=#net8}
N 1530 -440 1530 -160 {
lab=#net9}
N 1550 -440 1550 -340 {
lab=#net10}
N 1590 -440 1590 -160 {
lab=#net11}
N 1610 -440 1610 -340 {
lab=#net12}
N 1230 -100 1590 -100 {
lab=Voutp}
N 1310 -280 1670 -280 {
lab=Voutp}
N 290 -200 290 -190 {
lab=o6}
N 290 -240 290 -200 {
lab=o6}
N 1250 -440 1250 -400 {
lab=INp}
N -20 -220 -20 -200 {
lab=GND}
N -40 -310 -20 -310 {
lab=Voutp}
N -20 -310 -20 -280 {
lab=Voutp}
N 1770 -540 1820 -540 {
lab=VDD}
N 1890 -450 1890 -170 {
lab=#net13}
N 1910 -450 1910 -350 {
lab=#net14}
N 1950 -450 1950 -170 {
lab=#net15}
N 1970 -450 1970 -350 {
lab=#net16}
N 2010 -450 2010 -170 {
lab=#net17}
N 2030 -450 2030 -350 {
lab=#net18}
N 2070 -450 2070 -170 {
lab=#net19}
N 2090 -450 2090 -350 {
lab=#net20}
N 2130 -450 2130 -170 {
lab=#net21}
N 2150 -450 2150 -350 {
lab=#net22}
N 2190 -450 2190 -170 {
lab=#net23}
N 2210 -450 2210 -350 {
lab=#net24}
N 1830 -110 2190 -110 {
lab=Voutp2}
N 1910 -290 2270 -290 {
lab=Voutp2}
N 1850 -450 1850 -410 {
lab=INp2}
N 600 -260 600 -200 {
lab=INn2}
N 640 -210 640 -190 {
lab=#net25}
N 660 -210 660 -190 {
lab=#net26}
N 700 -210 700 -190 {
lab=#net27}
N 720 -210 720 -190 {
lab=#net28}
N 760 -210 760 -190 {
lab=#net29}
N 820 -210 820 -190 {
lab=#net30}
N 840 -210 840 -190 {
lab=#net31}
N 880 -210 880 -190 {
lab=#net32}
N 900 -250 900 -230 {
lab=#net33}
N 940 -210 940 -190 {
lab=#net34}
N 960 -250 960 -230 {
lab=#net35}
N 670 -390 670 -350 {
lab=Voutn2}
N 900 -230 900 -190 {
lab=#net33}
N 960 -230 960 -190 {
lab=#net35}
N 670 -350 670 -340 {
lab=Voutn2}
N 600 -200 600 -190 {
lab=INn2}
N 550 -100 570 -100 {
lab=GND}
N 660 -250 660 -210 {
lab=#net26}
N 720 -250 720 -210 {
lab=#net28}
N 840 -250 840 -210 {
lab=#net31}
N 640 -310 640 -270 {
lab=Voutn2}
N 640 -310 660 -310 {
lab=Voutn2}
N 670 -340 670 -310 {
lab=Voutn2}
N 640 -270 880 -270 {
lab=Voutn2}
N 660 -310 900 -310 {
lab=Voutn2}
N 780 -250 780 -240 {
lab=#net36}
N 900 -310 960 -310 {
lab=Voutn2}
N 880 -270 940 -270 {
lab=Voutn2}
N 780 -200 780 -190 {
lab=#net36}
N 780 -240 780 -200 {
lab=#net36}
N -190 -220 -190 -200 {
lab=GND}
N -210 -310 -190 -310 {
lab=Voutn2}
N -190 -310 -190 -280 {
lab=Voutn2}
N -90 -220 -90 -200 {
lab=GND}
N -110 -310 -90 -310 {
lab=Voutp2}
N -90 -310 -90 -280 {
lab=Voutp2}
N 840 -1040 840 -1020 {
lab=ISBCS}
N 1070 -970 1070 -950 {
lab=INp2_}
N 1040 -970 1040 -950 {
lab=INp_}
N 1070 -810 1070 -780 {
lab=INn2_}
N 1040 -810 1040 -780 {
lab=INn_}
N 940 -910 960 -910 {
lab=VDD}
N 940 -850 960 -850 {
lab=GND}
N 840 -1120 840 -1100 {
lab=VDD}
N 1070 -780 1120 -780 {
lab=INn2_}
N 1070 -970 1130 -970 {
lab=INp2_}
N 1040 -780 1040 -750 {
lab=INn_}
N 1040 -690 1040 -660 {
lab=INn}
N 1120 -720 1120 -690 {
lab=INn2}
N 1040 -1080 1040 -1050 {
lab=INp}
N 1040 -990 1040 -970 {
lab=INp_}
N 1130 -1060 1130 -1030 {
lab=INp2}
N 840 -1020 920 -1020 {
lab=ISBCS}
N 920 -1020 990 -1020 {
lab=ISBCS}
N 990 -1020 990 -950 {
lab=ISBCS}
C {devices/code_shown.sym} -130 -980 0 0 {name=MODELS only_toplevel=true
format="tcleval( @value )"
value="
.include $::180MCU_MODELS/design.ngspice
.lib $::180MCU_MODELS/sm141064.ngspice typical
*.param sw_stat_mismatch=1
*.param sw_stat_global=1

*.include /home/lci-ufsc/Desktop/Bracolin/TIA_Filter/CurrentMirrors/layout/spice/CM_input_pex.spice

*subckt CM_input ISBCS INP INP2 INN INN2 VDD VSS

*Xdut ISBCS INp_ INp2_ INn_ INn2_ VDD 0 CM_input
"}
C {devices/vsource.sym} -400 -250 0 0 {name=Vdd value=3.3
}
C {devices/gnd.sym} -400 -200 0 1 {name=l1 lab=GND
}
C {devices/code_shown.sym} -1280 -130 0 0 {name=NGSPICE only_toplevel=true
value="
.option gmin=1e-15
*.option abstol=1e-18
*.option retol=1e-15

.param i_sbcs=1n

.control
save all

let sample_num = 0

*while sample_num<151

*reset
op
remzerovec 
write TB_CM.raw
*wrdata /home/gmaranhao/Desktop/Bracolin/TIA_Filter/CurrentMirrors/out_cm.txt i(V1) i(V2) i(V3) i(V4) i(V5) i(V6) i(V7) i(V8) i(V9) i(V
10) i(V11) i(V12)
set appendwrite

dc Vo 0 3.3 10m
remzerovec 
write TB_CM.raw
set appendwrite

*let sample_num = sample_num + 1
*end


.endc
.save all
"}
C {devices/lab_wire.sym} -400 -310 0 0 {name=p1 sig_type=std_logic lab=VDD
}
C {devices/ammeter.sym} 150 -240 2 0 {name=V1 savecurrent=true
}
C {devices/ammeter.sym} 170 -280 2 0 {name=V2 savecurrent=true
}
C {devices/ammeter.sym} 210 -240 2 0 {name=V3 savecurrent=true
}
C {devices/ammeter.sym} 230 -280 2 0 {name=V4 savecurrent=true
}
C {devices/ammeter.sym} 270 -240 2 0 {name=V5 savecurrent=true
}
C {devices/ammeter.sym} 290 -280 2 0 {name=V6 savecurrent=true
}
C {devices/ammeter.sym} 330 -240 2 0 {name=V7 savecurrent=true
}
C {devices/ammeter.sym} 350 -280 2 0 {name=V8 savecurrent=true
}
C {devices/ammeter.sym} 390 -240 2 0 {name=V9 savecurrent=true
}
C {devices/ammeter.sym} 410 -280 2 0 {name=V10 savecurrent=true
}
C {devices/ammeter.sym} 450 -240 2 0 {name=V11 savecurrent=true
}
C {devices/ammeter.sym} 470 -280 2 0 {name=V12 savecurrent=true
}
C {devices/lab_wire.sym} 150 -200 0 0 {name=p10 sig_type=std_logic lab=o1
}
C {devices/lab_wire.sym} 170 -200 0 1 {name=p11 sig_type=std_logic lab=o2
}
C {devices/lab_wire.sym} 210 -200 0 1 {name=p12 sig_type=std_logic lab=o3
}
C {devices/lab_wire.sym} 230 -200 0 1 {name=p13 sig_type=std_logic lab=o4
}
C {devices/lab_wire.sym} 270 -200 0 1 {name=p14 sig_type=std_logic lab=o5
}
C {devices/lab_wire.sym} 290 -200 0 1 {name=p15 sig_type=std_logic lab=o6
}
C {devices/lab_wire.sym} 330 -200 0 1 {name=p16 sig_type=std_logic lab=o7
}
C {devices/lab_wire.sym} 350 -200 0 1 {name=p17 sig_type=std_logic lab=o8
}
C {devices/lab_wire.sym} 390 -200 0 1 {name=p18 sig_type=std_logic lab=o9
}
C {devices/lab_wire.sym} 410 -240 0 1 {name=p19 sig_type=std_logic lab=o10
}
C {devices/lab_wire.sym} 450 -200 0 0 {name=p20 sig_type=std_logic lab=o11
}
C {devices/lab_wire.sym} 470 -240 0 1 {name=p21 sig_type=std_logic lab=o12
}
C {devices/vsource.sym} -310 -250 0 0 {name=Vo value=1.65
}
C {devices/gnd.sym} -310 -200 0 1 {name=l3 lab=GND
}
C {devices/lab_wire.sym} -310 -310 0 0 {name=p22 sig_type=std_logic lab=Voutn
}
C {devices/lab_wire.sym} 180 -390 0 0 {name=p9 sig_type=std_logic lab=Voutn
}
C {devices/launcher.sym} 115 485 0 0 {name=h1
descr="Click left mouse button here with control key
pressed to load/unload waveforms in graph."
tclcommand="
xschem raw_read $netlist_dir/[file tail [file rootname [xschem get current_name]]].raw dc
"
}
C {CurrentMirrors/CM_n_net.sym} -180 160 0 0 {name=x1
}
C {devices/gnd.sym} 60 -100 0 1 {name=l2 lab=GND
}
C {devices/ammeter.sym} 1290 -130 0 0 {name=V13 savecurrent=true
}
C {devices/ammeter.sym} 1310 -310 0 0 {name=V14 savecurrent=true
}
C {devices/launcher.sym} 1345 445 0 0 {name=h4
descr="Click left mouse button here with control key
pressed to load/unload waveforms in graph."
tclcommand="
xschem raw_read $netlist_dir/[file tail [file rootname [xschem get current_name]]].raw dc
"
}
C {CurrentMirrors/CM_p_net.sym} 1460 -400 0 0 {name=x2
}
C {devices/lab_wire.sym} 1170 -530 0 0 {name=p5 sig_type=std_logic lab=VDD
}
C {devices/ammeter.sym} 1350 -130 0 0 {name=V15 savecurrent=true
}
C {devices/ammeter.sym} 1370 -310 0 0 {name=V16 savecurrent=true
}
C {devices/ammeter.sym} 1410 -130 0 0 {name=V17 savecurrent=true
}
C {devices/ammeter.sym} 1430 -310 0 0 {name=V18 savecurrent=true
}
C {devices/ammeter.sym} 1470 -130 0 0 {name=V19 savecurrent=true
}
C {devices/ammeter.sym} 1490 -310 0 0 {name=V20 savecurrent=true
}
C {devices/ammeter.sym} 1530 -130 0 0 {name=V21 savecurrent=true
}
C {devices/ammeter.sym} 1550 -310 0 0 {name=V22 savecurrent=true
}
C {devices/ammeter.sym} 1590 -130 0 0 {name=V23 savecurrent=true
}
C {devices/ammeter.sym} 1610 -310 0 0 {name=V24 savecurrent=true
}
C {devices/lab_wire.sym} 110 -260 0 1 {name=p26 sig_type=std_logic lab=INn
}
C {devices/lab_wire.sym} 1670 -280 0 1 {name=p3 sig_type=std_logic lab=Voutp}
C {devices/lab_wire.sym} 1250 -400 0 0 {name=p7 sig_type=std_logic lab=INp
}
C {devices/lab_wire.sym} 1240 -100 0 0 {name=p6 sig_type=std_logic lab=Voutp
}
C {devices/gnd.sym} -20 -200 0 1 {name=l5 lab=GND
}
C {devices/lab_wire.sym} -20 -310 0 0 {name=p27 sig_type=std_logic lab=Voutp
}
C {devices/ammeter.sym} 1890 -140 0 0 {name=V28 savecurrent=true
}
C {devices/ammeter.sym} 1910 -320 0 0 {name=V29 savecurrent=true
}
C {CurrentMirrors/CM_p_net.sym} 2060 -410 0 0 {name=x3
}
C {devices/lab_wire.sym} 1770 -540 0 0 {name=p28 sig_type=std_logic lab=VDD
}
C {devices/ammeter.sym} 1950 -140 0 0 {name=V30 savecurrent=true
}
C {devices/ammeter.sym} 1970 -320 0 0 {name=V31 savecurrent=true
}
C {devices/ammeter.sym} 2010 -140 0 0 {name=V32 savecurrent=true
}
C {devices/ammeter.sym} 2030 -320 0 0 {name=V33 savecurrent=true
}
C {devices/ammeter.sym} 2070 -140 0 0 {name=V34 savecurrent=true
}
C {devices/ammeter.sym} 2090 -320 0 0 {name=V35 savecurrent=true
}
C {devices/ammeter.sym} 2130 -140 0 0 {name=V36 savecurrent=true
}
C {devices/ammeter.sym} 2150 -320 0 0 {name=V37 savecurrent=true
}
C {devices/ammeter.sym} 2190 -140 0 0 {name=V38 savecurrent=true
}
C {devices/ammeter.sym} 2210 -320 0 0 {name=V39 savecurrent=true
}
C {devices/lab_wire.sym} 2270 -290 0 1 {name=p29 sig_type=std_logic lab=Voutp2}
C {devices/lab_wire.sym} 1850 -410 0 0 {name=p30 sig_type=std_logic lab=INp2
}
C {devices/lab_wire.sym} 1840 -110 0 0 {name=p31 sig_type=std_logic lab=Voutp2
}
C {devices/ammeter.sym} 640 -240 2 0 {name=V40 savecurrent=true
}
C {devices/ammeter.sym} 660 -280 2 0 {name=V41 savecurrent=true
}
C {devices/ammeter.sym} 700 -240 2 0 {name=V42 savecurrent=true
}
C {devices/ammeter.sym} 720 -280 2 0 {name=V43 savecurrent=true
}
C {devices/ammeter.sym} 760 -240 2 0 {name=V44 savecurrent=true
}
C {devices/ammeter.sym} 780 -280 2 0 {name=V45 savecurrent=true
}
C {devices/ammeter.sym} 820 -240 2 0 {name=V46 savecurrent=true
}
C {devices/ammeter.sym} 840 -280 2 0 {name=V47 savecurrent=true
}
C {devices/ammeter.sym} 880 -240 2 0 {name=V48 savecurrent=true
}
C {devices/ammeter.sym} 900 -280 2 0 {name=V49 savecurrent=true
}
C {devices/ammeter.sym} 960 -280 2 0 {name=V51 savecurrent=true
}
C {devices/lab_wire.sym} 670 -390 0 0 {name=p44 sig_type=std_logic lab=Voutn2
}
C {CurrentMirrors/CM_n_net.sym} 310 160 0 0 {name=x4
}
C {devices/gnd.sym} 550 -100 0 1 {name=l6 lab=GND
}
C {devices/lab_wire.sym} 600 -260 0 1 {name=p45 sig_type=std_logic lab=INn2
}
C {devices/ammeter.sym} 940 -240 2 0 {name=V50 savecurrent=true
}
C {devices/launcher.sym} 380 -1160 0 0 {name=h2
descr="Annotate OP" 
tclcommand="set show_hidden_texts 1; xschem annotate_op"
spice_ignore=true}
C {devices/isource.sym} 840 -1070 0 1 {name=I1 value=\{i_sbcs\}
}
C {devices/gnd.sym} -190 -200 0 1 {name=l7 lab=GND
}
C {devices/lab_wire.sym} -190 -310 0 0 {name=p35 sig_type=std_logic lab=Voutn2}
C {devices/gnd.sym} -90 -200 0 1 {name=l8 lab=GND
}
C {devices/lab_wire.sym} -90 -310 0 0 {name=p36 sig_type=std_logic lab=Voutp2
}
C {devices/lab_wire.sym} 1040 -1080 0 0 {name=p2 sig_type=std_logic lab=INp
}
C {devices/lab_wire.sym} 1130 -1060 0 1 {name=p4 sig_type=std_logic lab=INp2
}
C {devices/lab_wire.sym} 1120 -690 0 1 {name=p8 sig_type=std_logic lab=INn2
}
C {devices/lab_wire.sym} 1040 -660 0 0 {name=p23 sig_type=std_logic lab=INn
}
C {devices/lab_wire.sym} 940 -910 0 0 {name=p24 sig_type=std_logic lab=VDD
}
C {devices/gnd.sym} 940 -850 0 1 {name=l4 lab=GND
}
C {devices/lab_wire.sym} 840 -1120 0 0 {name=p25 sig_type=std_logic lab=VDD
}
C {devices/ammeter.sym} 1040 -720 0 0 {name=V25 savecurrent=true
}
C {devices/ammeter.sym} 1120 -750 0 0 {name=V26 savecurrent=true
}
C {devices/ammeter.sym} 1040 -1020 0 0 {name=V27 savecurrent=true
}
C {devices/ammeter.sym} 1130 -1000 0 0 {name=V52 savecurrent=true
}
C {devices/lab_wire.sym} 1040 -970 0 0 {name=p32 sig_type=std_logic lab=INp_}
C {devices/lab_wire.sym} 1080 -970 0 1 {name=p33 sig_type=std_logic lab=INp2_
}
C {devices/lab_wire.sym} 1040 -770 0 0 {name=p34 sig_type=std_logic lab=INn_
}
C {devices/lab_wire.sym} 1070 -790 0 1 {name=p37 sig_type=std_logic lab=INn2_
}
C {devices/lab_wire.sym} 890 -1020 0 0 {name=p38 sig_type=std_logic lab=ISBCS}
C {CurrentMirrors/CM_input.sym} 1060 -890 0 0 {name=x5}
C {devices/bsource.sym} -20 -250 0 0 {name=B1 VAR=V FUNC="V(Voutn)"}
C {devices/bsource.sym} -190 -250 0 1 {name=B2 VAR=V FUNC="V(Voutn)"}
C {devices/bsource.sym} -90 -250 0 1 {name=B3 VAR=V FUNC="V(Voutn)"}
