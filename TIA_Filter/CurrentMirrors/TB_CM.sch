v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
B 2 110 0 910 400 {flags=graph
y1=-2.3e-12
y2=2.5e-23
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=-0.66
x2=2.64
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0
node="i(v1)
i(v2)"
color="4 5"
dataset=-1
unitx=1
logx=0
logy=0
rainbow=1}
B 2 1200 250 2000 650 {flags=graph
y1=1e-12
y2=2.3e-12
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=-0.66
x2=2.64
divx=5
subdivx=1
xlabmag=1.0
ylabmag=1.0


dataset=-1
unitx=1
logx=0
logy=0
rainbow=1
color="4 5"
node="i(v13)
i(v14)"}
N -230 -220 -230 -200 {
lab=GND}
N -230 -310 -230 -280 {
lab=VDD}
N -250 -310 -230 -310 {
lab=VDD}
N 110 -260 110 -200 {
lab=INn}
N 150 -210 150 -190 {
lab=o1}
N 170 -210 170 -190 {
lab=o2}
N 210 -210 210 -190 {
lab=o3}
N 230 -210 230 -190 {
lab=o4}
N 270 -210 270 -190 {
lab=o5}
N 330 -210 330 -190 {
lab=o7}
N 350 -210 350 -190 {
lab=o8}
N 390 -210 390 -190 {
lab=o9}
N 410 -250 410 -230 {
lab=o10}
N 450 -210 450 -190 {
lab=o11}
N 470 -250 470 -230 {
lab=o12}
N 180 -390 180 -350 {
lab=Voutn}
N -140 -220 -140 -200 {
lab=GND}
N -160 -310 -140 -310 {
lab=Voutn}
N -140 -310 -140 -280 {
lab=Voutn}
N 410 -230 410 -190 {
lab=o10}
N 470 -230 470 -190 {
lab=o12}
N 180 -350 180 -340 {
lab=Voutn}
N 110 -200 110 -190 {
lab=INn}
N 60 -100 80 -100 {
lab=GND}
N 170 -250 170 -210 {
lab=o2}
N 230 -250 230 -210 {
lab=o4}
N 350 -250 350 -210 {
lab=o8}
N 150 -310 150 -270 {
lab=Voutn}
N 150 -310 170 -310 {
lab=Voutn}
N 180 -340 180 -310 {
lab=Voutn}
N 150 -270 390 -270 {
lab=Voutn}
N 170 -310 410 -310 {
lab=Voutn}
N 290 -250 290 -240 {
lab=o6}
N 410 -310 470 -310 {
lab=Voutn}
N 390 -270 450 -270 {
lab=Voutn}
N 1170 -400 1220 -400 {
lab=VDD}
N 1290 -310 1290 -30 {
lab=#net1}
N 1310 -310 1310 -210 {
lab=#net2}
N 1350 -310 1350 -30 {
lab=#net3}
N 1370 -310 1370 -210 {
lab=#net4}
N 1410 -310 1410 -30 {
lab=#net5}
N 1430 -310 1430 -210 {
lab=#net6}
N 1470 -310 1470 -30 {
lab=#net7}
N 1490 -310 1490 -210 {
lab=#net8}
N 1530 -310 1530 -30 {
lab=#net9}
N 1550 -310 1550 -210 {
lab=#net10}
N 1590 -310 1590 -30 {
lab=#net11}
N 1610 -310 1610 -210 {
lab=#net12}
N 1230 30 1590 30 {
lab=Voutp}
N 1310 -150 1670 -150 {
lab=Voutp}
N 290 -200 290 -190 {
lab=o6}
N 290 -240 290 -200 {
lab=o6}
N 1250 -310 1250 -270 {
lab=INp}
N -50 -220 -50 -200 {
lab=GND}
N -70 -310 -50 -310 {
lab=Voutp}
N -50 -310 -50 -280 {
lab=Voutp}
N 1770 -410 1820 -410 {
lab=VDD}
N 1890 -320 1890 -40 {
lab=#net13}
N 1910 -320 1910 -220 {
lab=#net14}
N 1950 -320 1950 -40 {
lab=#net15}
N 1970 -320 1970 -220 {
lab=#net16}
N 2010 -320 2010 -40 {
lab=#net17}
N 2030 -320 2030 -220 {
lab=#net18}
N 2070 -320 2070 -40 {
lab=#net19}
N 2090 -320 2090 -220 {
lab=#net20}
N 2130 -320 2130 -40 {
lab=#net21}
N 2150 -320 2150 -220 {
lab=#net22}
N 2190 -320 2190 -40 {
lab=#net23}
N 2210 -320 2210 -220 {
lab=#net24}
N 1830 20 2190 20 {
lab=Voutp2}
N 1910 -160 2270 -160 {
lab=Voutp}
N 1850 -320 1850 -280 {
lab=INp2}
N 600 -260 600 -200 {
lab=INn2}
N 640 -210 640 -190 {
lab=#net25}
N 660 -210 660 -190 {
lab=#net26}
N 700 -210 700 -190 {
lab=#net27}
N 720 -210 720 -190 {
lab=#net28}
N 760 -210 760 -190 {
lab=#net29}
N 820 -210 820 -190 {
lab=#net30}
N 840 -210 840 -190 {
lab=#net31}
N 880 -210 880 -190 {
lab=#net32}
N 900 -250 900 -230 {
lab=#net33}
N 940 -210 940 -190 {
lab=#net34}
N 960 -250 960 -230 {
lab=#net35}
N 670 -390 670 -350 {
lab=Voutn2}
N 900 -230 900 -190 {
lab=#net33}
N 960 -230 960 -190 {
lab=#net35}
N 670 -350 670 -340 {
lab=Voutn2}
N 600 -200 600 -190 {
lab=INn2}
N 550 -100 570 -100 {
lab=GND}
N 660 -250 660 -210 {
lab=#net26}
N 720 -250 720 -210 {
lab=#net28}
N 840 -250 840 -210 {
lab=#net31}
N 640 -310 640 -270 {
lab=Voutn2}
N 640 -310 660 -310 {
lab=Voutn2}
N 670 -340 670 -310 {
lab=Voutn2}
N 640 -270 880 -270 {
lab=Voutn2}
N 660 -310 900 -310 {
lab=Voutn2}
N 780 -250 780 -240 {
lab=#net36}
N 900 -310 960 -310 {
lab=Voutn2}
N 880 -270 940 -270 {
lab=Voutn2}
N 780 -200 780 -190 {
lab=#net36}
N 780 -240 780 -200 {
lab=#net36}
N 700 -880 700 -840 {
lab=Vg}
N 740 -810 840 -810 {
lab=Vg}
N 700 -860 760 -860 {
lab=Vg}
N 760 -860 760 -810 {
lab=Vg}
N 690 -810 700 -810 {
lab=GND}
N 880 -810 890 -810 {
lab=GND}
N 700 -970 700 -940 {
lab=VDD}
N 1130 -810 1140 -810 {
lab=GND}
N 1070 -810 1090 -810 {
lab=Vg}
N 1130 -1110 1130 -1080 {
lab=VDD}
N 1130 -1110 1290 -1110 {
lab=VDD}
N 1290 -1110 1290 -1080 {
lab=VDD}
N 1170 -1050 1250 -1050 {
lab=#net37}
N 1130 -920 1190 -920 {
lab=#net37}
N 1190 -970 1190 -920 {
lab=#net37}
N 1120 -1050 1130 -1050 {
lab=VDD}
N 1120 -1110 1120 -1050 {
lab=VDD}
N 1120 -1110 1130 -1110 {
lab=VDD}
N 1290 -940 1290 -900 {
lab=#net38}
N 880 -920 880 -910 {
lab=INp}
N 880 -850 880 -840 {
lab=#net39}
N 1130 -850 1130 -840 {
lab=#net40}
N 1290 -840 1290 -820 {
lab=INn}
N 1290 -1050 1300 -1050 {
lab=VDD}
N 1300 -1110 1300 -1050 {
lab=VDD}
N 1290 -1110 1300 -1110 {
lab=VDD}
N 690 -730 700 -730 {
lab=GND}
N 700 -780 700 -760 {
lab=#net41}
N 880 -730 890 -730 {
lab=GND}
N 880 -780 880 -760 {
lab=#net42}
N 740 -730 760 -730 {
lab=Vg}
N 1130 -780 1130 -760 {
lab=#net43}
N 1130 -730 1140 -730 {
lab=GND}
N 810 -700 1130 -700 {
lab=GND}
N 690 -730 690 -700 {
lab=GND}
N 690 -700 700 -700 {
lab=GND}
N 890 -810 890 -700 {
lab=GND}
N 1140 -810 1140 -700 {
lab=GND}
N 1130 -700 1140 -700 {
lab=GND}
N 690 -810 690 -730 {
lab=GND}
N 840 -810 840 -730 {
lab=Vg}
N 760 -810 760 -730 {
lab=Vg}
N 1090 -810 1090 -730 {
lab=Vg}
N 1190 -1050 1190 -970 {
lab=#net37}
N 1170 -980 1190 -980 {
lab=#net37}
N 1130 -1020 1130 -1010 {
lab=#net44}
N 1130 -950 1130 -930 {
lab=#net37}
N 1130 -930 1130 -910 {
lab=#net37}
N 1120 -1050 1120 -980 {
lab=VDD}
N 1120 -980 1130 -980 {
lab=VDD}
N 1250 -1050 1250 -980 {
lab=#net37}
N 1290 -1020 1290 -1010 {
lab=#net45}
N 1290 -950 1290 -940 {
lab=#net38}
N 1300 -1050 1300 -980 {
lab=VDD}
N 1290 -980 1300 -980 {
lab=VDD}
N 1000 -810 1010 -810 {
lab=GND}
N 1000 -920 1000 -910 {
lab=INp2}
N 1000 -850 1000 -840 {
lab=#net46}
N 1000 -730 1010 -730 {
lab=GND}
N 1000 -780 1000 -760 {
lab=#net47}
N 1010 -810 1010 -700 {
lab=GND}
N 960 -810 960 -730 {
lab=Vg}
N 940 -810 960 -810 {
lab=Vg}
N 700 -700 810 -700 {
lab=GND}
N 1290 -1110 1450 -1110 {
lab=VDD}
N 1450 -1110 1450 -1080 {
lab=VDD}
N 1450 -940 1450 -900 {
lab=#net48}
N 1450 -840 1450 -820 {
lab=INn2}
N 1450 -1050 1460 -1050 {
lab=VDD}
N 1460 -1110 1460 -1050 {
lab=VDD}
N 1450 -1110 1460 -1110 {
lab=VDD}
N 1410 -1050 1410 -980 {
lab=#net37}
N 1450 -1020 1450 -1010 {
lab=#net49}
N 1450 -950 1450 -940 {
lab=#net48}
N 1460 -1050 1460 -980 {
lab=VDD}
N 1450 -980 1460 -980 {
lab=VDD}
N 1220 -1090 1220 -1050 {
lab=#net37}
N 1220 -1090 1410 -1090 {
lab=#net37}
N 1410 -1090 1410 -1050 {
lab=#net37}
N -130 -50 -130 -30 {
lab=GND}
N -150 -140 -130 -140 {
lab=Voutn2}
N -130 -140 -130 -110 {
lab=Voutn2}
N -40 -50 -40 -30 {
lab=GND}
N -60 -140 -40 -140 {
lab=Voutp2}
N -40 -140 -40 -110 {
lab=Voutp2}
C {devices/code_shown.sym} -1290 -340 0 0 {name=MODELS only_toplevel=true
format="tcleval( @value )"
value="
.include $::180MCU_MODELS/design.ngspice
.lib $::180MCU_MODELS/sm141064.ngspice typical
*.param sw_stat_mismatch=1
*.param sw_stat_global=1
"}
C {devices/vsource.sym} -230 -250 0 0 {name=Vdd value=3.3
}
C {devices/gnd.sym} -230 -200 0 1 {name=l1 lab=GND
}
C {devices/code_shown.sym} -1280 -130 0 0 {name=NGSPICE only_toplevel=true
value="
.option gmin=1e-16
*.option abstol=1e-18
*.option retol=1e-15

.param i_sbcs=1n

.control
save all

let sample_num = 0

*while sample_num<151

*reset
op
remzerovec 
write TB_CM.raw
*wrdata /home/gmaranhao/Desktop/Bracolin/TIA_Filter/CurrentMirrors/out_cm.txt i(V1) i(V2) i(V3) i(V4) i(V5) i(V6) i(V7) i(V8) i(V9) i(V
10) i(V11) i(V12)
set appendwrite

*dc Vo 0 3.3 10m
*remzerovec 
*write TB_CM.raw
*set appendwrite

*let sample_num = sample_num + 1
*end


.endc
.save all
"}
C {devices/lab_wire.sym} -230 -310 0 0 {name=p1 sig_type=std_logic lab=VDD
}
C {devices/ammeter.sym} 150 -240 2 0 {name=V1 savecurrent=true
}
C {devices/ammeter.sym} 170 -280 2 0 {name=V2 savecurrent=true
}
C {devices/ammeter.sym} 210 -240 2 0 {name=V3 savecurrent=true
}
C {devices/ammeter.sym} 230 -280 2 0 {name=V4 savecurrent=true
}
C {devices/ammeter.sym} 270 -240 2 0 {name=V5 savecurrent=true
}
C {devices/ammeter.sym} 290 -280 2 0 {name=V6 savecurrent=true
}
C {devices/ammeter.sym} 330 -240 2 0 {name=V7 savecurrent=true
}
C {devices/ammeter.sym} 350 -280 2 0 {name=V8 savecurrent=true
}
C {devices/ammeter.sym} 390 -240 2 0 {name=V9 savecurrent=true
}
C {devices/ammeter.sym} 410 -280 2 0 {name=V10 savecurrent=true
}
C {devices/ammeter.sym} 450 -240 2 0 {name=V11 savecurrent=true
}
C {devices/ammeter.sym} 470 -280 2 0 {name=V12 savecurrent=true
}
C {devices/lab_wire.sym} 150 -200 0 0 {name=p10 sig_type=std_logic lab=o1
}
C {devices/lab_wire.sym} 170 -200 0 1 {name=p11 sig_type=std_logic lab=o2
}
C {devices/lab_wire.sym} 210 -200 0 1 {name=p12 sig_type=std_logic lab=o3
}
C {devices/lab_wire.sym} 230 -200 0 1 {name=p13 sig_type=std_logic lab=o4
}
C {devices/lab_wire.sym} 270 -200 0 1 {name=p14 sig_type=std_logic lab=o5
}
C {devices/lab_wire.sym} 290 -200 0 1 {name=p15 sig_type=std_logic lab=o6
}
C {devices/lab_wire.sym} 330 -200 0 1 {name=p16 sig_type=std_logic lab=o7
}
C {devices/lab_wire.sym} 350 -200 0 1 {name=p17 sig_type=std_logic lab=o8
}
C {devices/lab_wire.sym} 390 -200 0 1 {name=p18 sig_type=std_logic lab=o9
}
C {devices/lab_wire.sym} 410 -240 0 1 {name=p19 sig_type=std_logic lab=o10
}
C {devices/lab_wire.sym} 450 -200 0 0 {name=p20 sig_type=std_logic lab=o11
}
C {devices/lab_wire.sym} 470 -240 0 1 {name=p21 sig_type=std_logic lab=o12
}
C {devices/vsource.sym} -140 -250 0 0 {name=Vo value=1.65
}
C {devices/gnd.sym} -140 -200 0 1 {name=l3 lab=GND
}
C {devices/lab_wire.sym} -140 -310 0 0 {name=p22 sig_type=std_logic lab=Voutn
}
C {devices/lab_wire.sym} 180 -390 0 0 {name=p9 sig_type=std_logic lab=Voutn
}
C {devices/launcher.sym} 115 485 0 0 {name=h1
descr="Click left mouse button here with control key
pressed to load/unload waveforms in graph."
tclcommand="
xschem raw_read $netlist_dir/[file tail [file rootname [xschem get current_name]]].raw dc
"
}
C {CurrentMirrors/CM_n_net.sym} -180 160 0 0 {name=x1
}
C {devices/gnd.sym} 60 -100 0 1 {name=l2 lab=GND
}
C {devices/ammeter.sym} 1290 0 0 0 {name=V13 savecurrent=true
}
C {devices/ammeter.sym} 1310 -180 0 0 {name=V14 savecurrent=true
}
C {devices/launcher.sym} 1210 180 0 0 {name=h3
descr="Annotate OP" 
tclcommand="set show_hidden_texts 1; xschem annotate_op"
}
C {devices/launcher.sym} 1285 715 0 0 {name=h4
descr="Click left mouse button here with control key
pressed to load/unload waveforms in graph."
tclcommand="
xschem raw_read $netlist_dir/[file tail [file rootname [xschem get current_name]]].raw dc
"
}
C {CurrentMirrors/CM_p_net.sym} 1460 -270 0 0 {name=x2
}
C {devices/lab_wire.sym} 1170 -400 0 0 {name=p5 sig_type=std_logic lab=VDD
}
C {devices/ammeter.sym} 1350 0 0 0 {name=V15 savecurrent=true
}
C {devices/ammeter.sym} 1370 -180 0 0 {name=V16 savecurrent=true
}
C {devices/ammeter.sym} 1410 0 0 0 {name=V17 savecurrent=true
}
C {devices/ammeter.sym} 1430 -180 0 0 {name=V18 savecurrent=true
}
C {devices/ammeter.sym} 1470 0 0 0 {name=V19 savecurrent=true
}
C {devices/ammeter.sym} 1490 -180 0 0 {name=V20 savecurrent=true
}
C {devices/ammeter.sym} 1530 0 0 0 {name=V21 savecurrent=true
}
C {devices/ammeter.sym} 1550 -180 0 0 {name=V22 savecurrent=true
}
C {devices/ammeter.sym} 1590 0 0 0 {name=V23 savecurrent=true
}
C {devices/ammeter.sym} 1610 -180 0 0 {name=V24 savecurrent=true
}
C {devices/lab_wire.sym} 110 -260 0 1 {name=p26 sig_type=std_logic lab=INn
}
C {devices/lab_wire.sym} 1670 -150 0 1 {name=p3 sig_type=std_logic lab=Voutp}
C {devices/lab_wire.sym} 1250 -270 0 0 {name=p7 sig_type=std_logic lab=INp
}
C {devices/lab_wire.sym} 1240 30 0 0 {name=p6 sig_type=std_logic lab=Voutp
}
C {devices/vsource.sym} -50 -250 0 0 {name=Vo1 value=1.65
}
C {devices/gnd.sym} -50 -200 0 1 {name=l5 lab=GND
}
C {devices/lab_wire.sym} -50 -310 0 0 {name=p27 sig_type=std_logic lab=Voutp
}
C {devices/ammeter.sym} 1890 -10 0 0 {name=V28 savecurrent=true
}
C {devices/ammeter.sym} 1910 -190 0 0 {name=V29 savecurrent=true
}
C {CurrentMirrors/CM_p_net.sym} 2060 -280 0 0 {name=x3
}
C {devices/lab_wire.sym} 1770 -410 0 0 {name=p28 sig_type=std_logic lab=VDD
}
C {devices/ammeter.sym} 1950 -10 0 0 {name=V30 savecurrent=true
}
C {devices/ammeter.sym} 1970 -190 0 0 {name=V31 savecurrent=true
}
C {devices/ammeter.sym} 2010 -10 0 0 {name=V32 savecurrent=true
}
C {devices/ammeter.sym} 2030 -190 0 0 {name=V33 savecurrent=true
}
C {devices/ammeter.sym} 2070 -10 0 0 {name=V34 savecurrent=true
}
C {devices/ammeter.sym} 2090 -190 0 0 {name=V35 savecurrent=true
}
C {devices/ammeter.sym} 2130 -10 0 0 {name=V36 savecurrent=true
}
C {devices/ammeter.sym} 2150 -190 0 0 {name=V37 savecurrent=true
}
C {devices/ammeter.sym} 2190 -10 0 0 {name=V38 savecurrent=true
}
C {devices/ammeter.sym} 2210 -190 0 0 {name=V39 savecurrent=true
}
C {devices/lab_wire.sym} 2270 -160 0 1 {name=p29 sig_type=std_logic lab=Voutp}
C {devices/lab_wire.sym} 1850 -280 0 0 {name=p30 sig_type=std_logic lab=INp2
}
C {devices/lab_wire.sym} 1840 20 0 0 {name=p31 sig_type=std_logic lab=Voutp2
}
C {devices/ammeter.sym} 640 -240 2 0 {name=V40 savecurrent=true
}
C {devices/ammeter.sym} 660 -280 2 0 {name=V41 savecurrent=true
}
C {devices/ammeter.sym} 700 -240 2 0 {name=V42 savecurrent=true
}
C {devices/ammeter.sym} 720 -280 2 0 {name=V43 savecurrent=true
}
C {devices/ammeter.sym} 760 -240 2 0 {name=V44 savecurrent=true
}
C {devices/ammeter.sym} 780 -280 2 0 {name=V45 savecurrent=true
}
C {devices/ammeter.sym} 820 -240 2 0 {name=V46 savecurrent=true
}
C {devices/ammeter.sym} 840 -280 2 0 {name=V47 savecurrent=true
}
C {devices/ammeter.sym} 880 -240 2 0 {name=V48 savecurrent=true
}
C {devices/ammeter.sym} 900 -280 2 0 {name=V49 savecurrent=true
}
C {devices/ammeter.sym} 960 -280 2 0 {name=V51 savecurrent=true
}
C {devices/lab_wire.sym} 670 -390 0 0 {name=p44 sig_type=std_logic lab=Voutn2
}
C {CurrentMirrors/CM_n_net.sym} 310 160 0 0 {name=x4
}
C {devices/gnd.sym} 550 -100 0 1 {name=l6 lab=GND
}
C {devices/lab_wire.sym} 600 -260 0 1 {name=p45 sig_type=std_logic lab=INn2
}
C {devices/ammeter.sym} 940 -240 2 0 {name=V50 savecurrent=true
}
C {devices/launcher.sym} 800 -1020 0 0 {name=h2
descr="Annotate OP" 
tclcommand="set show_hidden_texts 1; xschem annotate_op"
spice_ignore=true}
C {symbols/nfet_03v3.sym} 720 -810 0 1 {name=M10[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_03v3
spiceprefix=X
}
C {devices/isource.sym} 700 -910 0 1 {name=I1 value=\{i_sbcs\}
}
C {devices/lab_wire.sym} 810 -810 0 0 {name=p2 sig_type=std_logic lab=Vg
}
C {devices/lab_wire.sym} 1070 -810 0 0 {name=p4 sig_type=std_logic lab=Vg
}
C {symbols/nfet_03v3.sym} 860 -810 0 0 {name=M12[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_03v3
spiceprefix=X
}
C {symbols/pfet_03v3.sym} 1150 -1050 0 1 {name=M13[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_03v3
spiceprefix=X
}
C {symbols/nfet_03v3.sym} 1110 -810 0 0 {name=M14[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_03v3
spiceprefix=X
}
C {symbols/pfet_03v3.sym} 1270 -1050 0 0 {name=M15[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_03v3
spiceprefix=X
}
C {devices/lab_wire.sym} 700 -970 0 0 {name=p8 sig_type=std_logic lab=VDD
}
C {devices/lab_wire.sym} 880 -920 0 0 {name=p23 sig_type=std_logic lab=INp
}
C {devices/lab_wire.sym} 1290 -820 0 0 {name=p24 sig_type=std_logic lab=INn
}
C {devices/ammeter.sym} 880 -880 0 0 {name=V25 savecurrent=true
}
C {devices/ammeter.sym} 1130 -880 2 0 {name=V26 savecurrent=true
}
C {devices/ammeter.sym} 1290 -870 2 0 {name=V27 savecurrent=true
}
C {devices/lab_wire.sym} 1210 -1110 0 0 {name=p25 sig_type=std_logic lab=VDD
}
C {symbols/nfet_03v3.sym} 720 -730 0 1 {name=M16[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_03v3
spiceprefix=X
}
C {symbols/nfet_03v3.sym} 860 -730 0 0 {name=M17[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_03v3
spiceprefix=X
}
C {symbols/nfet_03v3.sym} 1110 -730 0 0 {name=M18[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_03v3
spiceprefix=X
}
C {devices/gnd.sym} 810 -700 0 1 {name=l4 lab=GND
}
C {symbols/pfet_03v3.sym} 1150 -980 0 1 {name=M19[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_03v3
spiceprefix=X
}
C {symbols/pfet_03v3.sym} 1270 -980 0 0 {name=M20[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_03v3
spiceprefix=X
}
C {symbols/nfet_03v3.sym} 980 -810 0 0 {name=M21[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_03v3
spiceprefix=X
}
C {devices/lab_wire.sym} 1000 -920 0 0 {name=p32 sig_type=std_logic lab=INp2
}
C {devices/ammeter.sym} 1000 -880 0 0 {name=V52 savecurrent=true
}
C {symbols/nfet_03v3.sym} 980 -730 0 0 {name=M22[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=nfet_03v3
spiceprefix=X
}
C {devices/lab_wire.sym} 940 -810 0 0 {name=p33 sig_type=std_logic lab=Vg
}
C {symbols/pfet_03v3.sym} 1430 -1050 0 0 {name=M23[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_03v3
spiceprefix=X
}
C {devices/lab_wire.sym} 1450 -820 0 0 {name=p34 sig_type=std_logic lab=INn2
}
C {devices/ammeter.sym} 1450 -870 2 0 {name=V53 savecurrent=true
}
C {symbols/pfet_03v3.sym} 1430 -980 0 0 {name=M24[1:16]
L=6u
W=2u
nf=1
m=1
ad="'int((nf+1)/2) * W/nf * 0.18u'"
pd="'2*int((nf+1)/2) * (W/nf + 0.18u)'"
as="'int((nf+2)/2) * W/nf * 0.18u'"
ps="'2*int((nf+2)/2) * (W/nf + 0.18u)'"
nrd="'0.18u / W'" nrs="'0.18u / W'"
sa=0 sb=0 sd=0
model=pfet_03v3
spiceprefix=X
}
C {devices/vsource.sym} -130 -80 0 0 {name=Vo2 value=1.65
}
C {devices/gnd.sym} -130 -30 0 1 {name=l7 lab=GND
}
C {devices/lab_wire.sym} -130 -140 0 0 {name=p35 sig_type=std_logic lab=Voutn2}
C {devices/vsource.sym} -40 -80 0 0 {name=Vo3 value=1.65
}
C {devices/gnd.sym} -40 -30 0 1 {name=l8 lab=GND
}
C {devices/lab_wire.sym} -40 -140 0 0 {name=p36 sig_type=std_logic lab=Voutp2
}
